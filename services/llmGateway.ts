...
export class LLMGateway {
  ...
  private async generatePoeImage(prompt: string): Promise<string> {
    ...
    const extraBody: any = { aspect_ratio: this.settings.imageAspectRatio || "16:9" };
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Poe Image API Error: ${response.status}`);

        const data = await response.json();
        const content = data.choices?.[0]?.message?.content || "";

        // Improved Markdown Image Extraction
